...
# 형태소 분석을 통한 명사 추출 루틴                 #
#####################################################
nws <- read.csv("scraped_articles_joongang[2020-08-26(13,35,04)].csv", header = TRUE, fileEncoding = "UTF-8", stringsAsFactors=F)
...
nws <- read.csv("ex_rem2_noun_articles_joongang(corona)(2000).csv", header = TRUE, fileEncoding = "UTF-8", stringsAsFactors=F)
...
selected <- data[1:100,] #상위 100개의 단어를 선택
...
wordcloud2(selected, size=1.0, shape = 'star', col="random-dark", rotateRatio = 0)
